...
protocol VerifyRCVProtocol {
    func onVerify(code : String)
}

class RCV_VerifyPaymentVC: BaseVC {
    ...
    var delegate : VerifyRCVProtocol?
    ...
    func sendVerificationCode() {
        self.showLoading()
        self.getApiManager().resendRcvOtpCode(token: getAccessToken(), rcvGuid: self.rcvGuid ?? "") { (response) in
            self.hideLoading()
            self.fieldPin.text = ""
            self.enableNext(flag: false)
            self.startTimer()
...
extension RCV_VerifyPaymentVC : UITextFieldDelegate {
