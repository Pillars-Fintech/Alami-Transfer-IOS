...
protocol VerifyAFSProtocol {
    func onVerify(code : String)
}

class afs_VerifyPaymentVC: BaseVC {
    ...
    var delegate : VerifyAFSProtocol?
    ...
    func sendVerificationCode() {
        self.showLoading()
        self.getApiManager().resendAfsOtpCode(token: getAccessToken(), afsGuid: self.afsGuid ?? "") { (response) in
            self.hideLoading()
            self.fieldPin.text = ""
            self.enableNext(flag: false)
            self.startTimer()
...
extension afs_VerifyPaymentVC : UITextFieldDelegate {
