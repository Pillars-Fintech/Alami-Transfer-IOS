...
class mg_PayInMethodVC: BaseVC, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        print("self.sendModel!\(self.sendModel?.amount)")
        print(" self.selectedMethod?.code!\(self.selectedMethod?.code)")
    ...
    func fillData() {
        ...
        App.shared.sendMoneyMG?.payInOption = self.selectedMethod
        
        
        
     
        App.shared.sendMoneyMG?.senderRegion = self.fieldSenderRegion.text ?? ""
        App.shared.sendMoneyMG?.senderCity = self.fieldSenderCity.text ?? ""
        App.shared.sendMoneyMG?.senderStreet = self.fieldSenderStreet.text ?? ""
        App.shared.sendMoneyMG?.senderBuildingName = self.fieldSenderBuildingName.text ?? ""
        App.shared.sendMoneyMG?.senderBuildingNumber = self.fieldSenderBuildingNumber.text ?? ""
        App.shared.sendMoneyMG?.senderApartment = self.fieldSenderApartment.text ?? ""
        App.shared.sendMoneyMG?.senderAddress = self.fieldSenderAddress.text ?? ""
        
        App.shared.sendMoneyMG?.ReceiverRegion = self.fieldReceiverRegion.text ?? ""
        App.shared.sendMoneyMG?.ReceiverCity = self.fieldReceiverCity.text ?? ""
        App.shared.sendMoneyMG?.ReceiverStreet = self.fieldReceiverStreet.text ?? ""
        App.shared.sendMoneyMG?.ReceiverBuildingName = self.fieldReceiverBuildingName.text ?? ""
        App.shared.sendMoneyMG?.ReceiverBuildingNumber = self.fieldReceiverBuildingNumber.text ?? ""
        App.shared.sendMoneyMG?.ReceiverApartment = self.fieldReceiverApartment.text ?? ""
        App.shared.sendMoneyMG?.ReceiverAddress = self.fieldReceiverAddress.text ?? ""
        
        App.shared.sendMoneyMG?.walletProvider = self.selectedWallet
        App.shared.sendMoneyMG?.walletNumber = self.fieldWalletNumber.text ?? ""
    }
    ...
    func validate() -> Bool {
        ...
        if (self.selectedMethod?.code == "03" || self.selectedMethod?.code == "04") { //door to door or collect cash
...
//            }
    ...
    func validateFields() {
        ...
        if (self.selectedMethod?.code == "03" || self.selectedMethod?.code == "03") { //door to door or collect cash
            if (self.fieldSenderRegion.text?.count ?? 0 == 0) {
                self.enableNext(flag: false)
                return
            }
...
//            }
    ...
    @IBAction func selectWalletProvider(_ sender: Any) {
        self.showLoading()
        self.getApiManager().getWalletProviders(token: self.getAccessToken(), isInternal: false) { (response) in
            self.hideLoading()
    ...
    @IBAction func continueAction(_ sender: Any) {
        if(self.validate()) {
            self.fillData()
//         self.pushVC(name: "mg_PayOutMethodVC", sb: Constants.STORYBOARDS.money_gram)

//            loadServiceOptions()
//            self.pushVC(name: "mg_PayOutMethodVC", sb: Constants.STORYBOARDS.money_gram)
//            self.pushVC(name: "MoneyGramViewController", sb: Constants.STORYBOARDS.moneyGramTest)
...
extension mg_PayInMethodVC : UITextFieldDelegate {
